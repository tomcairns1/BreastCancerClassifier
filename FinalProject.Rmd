...
findPrediction <- function(knn_pred, svm_pred, ann_pred) {
  ...
  predictions <- (as.numeric(factor(knn_pred, levels = bc_order)) - 1) * knn.w + 
    (as.numeric(factor(svm.pred, levels = bc_order)) - 1) * svm.w +
    (as.numeric(factor(ann.pred, levels = bc_order)) - 1) * ann.w
