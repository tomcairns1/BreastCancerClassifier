...
```{r}
# Create kNN model
bc.knn <- knn(train = dplyr::select(bc_train.oversampled, -cancer_type), 
              test = dplyr::select(bc_test, -cancer_type), 
              cl = bc_train.oversampled$cancer_type, k = 2)

# Create Cross Table to verify accuracy
knn.cross <- CrossTable(x = bc_test$cancer_type, y = bc.knn, 
                        prop.chisq = F)
```
...
Here we can see the accuracy of our model is 88.7% with a kappa statistic of
0.4736. This is a much better fit of the data than the kNN model.
