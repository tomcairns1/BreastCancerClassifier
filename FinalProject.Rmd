...
max_its <- 5
layers <- rep(c(1, 2), max_its)
nodes <- rep(seq(1, max_its), 2)
nodes <- nodes[order(nodes)]
ann.acc <- data_frame(nodes = nodes, layers)
ann.acc$acc <- 0
ann.auc <- data_frame(nodes = nodes, layers)
ann.auc$auc <- 0
ann.k <- data_frame(nodes = nodes, layers)
ann.k$k <- 0

for (i in 1:max_its) {
  for (j in 1:2) {
  # Create the grid of hidden layers
  grid <- expand.grid(size = c(i, j), decay = 0)
  ...
  # Find accuracy
  ann.acc$acc[i * j] <- findAccuracy(ct$t)
  
  # Find auc
  ann.auc$auc[i * j] <- auc(as.numeric(factor(bc_test$cancer_type, levels = bc_order)),
                        as.numeric(factor(pred, levels = bc_order)))
  
  # Find kappa
  ann.k$k[i * j] <- Kappa(ct$t)$Unweighted[[1]]
  }
}
