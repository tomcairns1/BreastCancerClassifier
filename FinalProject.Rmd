...
```{r feature-selection-main-model}
# Convert cancer type to numeric
bc_train$cancer_type_numeric <- as.numeric(factor(bc_train$cancer_type))

# Create initial multiple logistic regression model
bc_train.reg <- glm(cancer_type_numeric ~ ., data = bc_train[,-c(169)], 
                    family = 'gaussian')
```

```{r stepwise-elimination, include=F}
