...
**I HAVE CHANGED THIS TO BINARY CLASSIFIER BC FUCK MIXED**
...
bc_train.reg <- glm(cancer_type_detailed_numeric ~ ., data = bc_train[,c(2:478)], family = 'gaussian')
```
...
# mwmote(bc_train.filter, numInstances = 3150, classAttr = bc_train$cancer_type_detailed)

# A simple oversampling
bmdalc <- filter(bc_train, 
                 cancer_type_detailed == 'Breast Mixed Ductal and Lobular Carcinoma')
bilc <- filter(bc_train, cancer_type_detailed == 'Breast Invasive Lobular Carcinoma')

# Add to the dataset
bc_train.oversampled <- rbind(bc_train, bmdalc, bmdalc, bmdalc, bmdalc, 
                              bmdalc, bmdalc, bilc, bilc, bilc, bilc, bilc, bilc, 
                              bilc, bilc, bilc, bilc)
...
Carcinoma, 100 instances of Breast Invasive Lobular Carcinoma, and 145 instances
of Breast Mixed Ductal and Lobular Carcinoma. I used oversampling to increase
the samples of the minority classes to be about the same as the majority class.
I needed to make 6 copies of the Breast Mixed Ductal and Lobular Carcinoma and
10 copies of the Breast Invasive Lobulr Carcinoma.
...
bc.knn <- knn(dplyr::select(bc_train.oversampled, -cancer_type_detailed), 
              dplyr::select(bc_test, -cancer_type_detailed), 
              cl = bc_train.oversampled$cancer_type_detailed, k = 3)
