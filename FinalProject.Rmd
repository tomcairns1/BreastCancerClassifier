...
```{r evaluate-nn}
grid <- expand.grid(size = 1, decay = 0)
...
runEnsemble <- function(df) {
  ...
  # Find the kappa values
  knn.kappa <- Kappa(CrossTable(knn.en, df$cancer_type, prop.chisq = F)$t)
  svm.kappa <- Kappa(CrossTable(svm.en, df$cancer_type, prop.chisq = F)$t)
  ann.kappa <- Kappa(CrossTable(ann.en, df$cancer_type, prop.chisq = F)$t)
  
    # Calculate the weights
  total_kappa <- sum(abs(knn.kappa), svm.kappa, ann.kappa) # knn might not be negative
  knn.w <- abs(knn.kappa / total_kappa)
  svm.w <- svm.kappa / total_kappa
  ann.w <- ann.kappa / total_kappa
...
findPrediction <- function(knn_pred, svm_pred, ann_pred) {
  ...
  predictions <- (as.numeric(knn_pred) - 1) * knn.w + 
    (as.numeric(svm.pred) - 1) * svm.w +
    (as.numeric(ann.pred) - 1) * ann.w
