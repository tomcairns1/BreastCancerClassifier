...
adaboost <- function(test_data, ml_1, ml_2, ml_3) {
  ...
  # Select samples
  obs <- sample(test_data, size = nrow(test_data), prob = w, replace = T)

  # Create predictions
  p1 <- predict(ml_1, obs)
  
  # Find error
  e <- sum(w[p1 == test_data$classes])
...
# Set the new weight
setNewWeight <- function(weight, prediction, test_data, observations)
```
