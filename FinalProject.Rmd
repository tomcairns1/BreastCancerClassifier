...
m1 <- bc[bc$cancer_type == 'Breast Invasive Lobular Carcinoma',][1,]
m1$ATR <- 0
m2 <- bc[bc$cancer_type == 'Breast Invasive Lobular Carcinoma',][2,]
m2$CDH1 <- 0
m3 <- bc[bc$cancer_type == 'Breast Invasive Lobular Carcinoma',][3,]
m3$GATA3 <- 0
...
```{r neural-net, include=FALSE, message=FALSE}
# Set seed and random number generator
RNGversion('3.5.2'); set.seed(123)
...
```{r evaluate-nn, include=FALSE, message=FALSE}
# Set seed and random number generator
RNGversion('3.5.2'); set.seed(123)
...
runEnsemble <- function(df) {
  ...
  knn.en <- knn(train = dplyr::select(bc_train.oversampled, -cancer_type),
              ...
              cl = as.character(bc_train.oversampled$cancer_type), k = 2)
  svm.en <- predict(bc_svm, df)
  ann.en <- predict(bc_ann, df)
  
  # Find the kappa values
  knn.kappa <- Kappa(CrossTable(knn.en, df$cancer_type, prop.chisq = F)$t)$Unweighted[[1]]
  svm.kappa <- Kappa(CrossTable(svm.en, df$cancer_type, prop.chisq = F)$t)$Unweighted[[1]]
  ann.kappa <- Kappa(CrossTable(ann.en, df$cancer_type, prop.chisq = F)$t)$Unweighted[[1]]
...
**HAVING AN ISSUE WHERE THE CLASSES ARE BACKWARD AGAIN**
**INCLUDE PRECISION AND RECALL FOR ALL MEASURES AND EXPLAIN THAT THE MINORITY**
**CASE IS GOING TO BE THE "POSITIVE" CASE SINCE THE MAJORITY CASE IS ESSENTIALLY**
**THE DEFAULT**
