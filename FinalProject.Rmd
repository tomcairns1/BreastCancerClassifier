...
```{r}
# Create kNN model
# bc.knn <- train(factor(cancer_type) ~ ., data = bc_train.oversampled, method = 'knn',
...
Here we can see the accuracy of our model is 83.8% with a kappa statistic of
0.21. This is a much better fit of the data than the kNN model.

```{r}
# Hypertune the model
bc_svm.hypertune <- train(cancer_type ~ ., data = bc_train.oversampled, 
                method = 'svmRadial')
bc_svm.pred.h1 <- predict(bc_svm.hypertune, bc_test)
...
# Create CrossTable
svm.cross.h1 <- CrossTable(x = bc_test$cancer_type, y = bc_svm.pred.h1,
                        prop.chisq = F)

# Find accuracy
accuracy.svm <- findAccuracy(svm.cross.h1$t)
paste('The accuracy of the model is:', round(accuracy.svm, 4))

# Find kappa
kappa.svm <- Kappa(svm.cross.h1$t)
paste('The Kappa Statistic is:', round(kappa.svm$Unweighted[[1]], 4))
```

Using the argument `preProcess = c('center', 'scale')` did not improve the model
performance (not pictured).
