...
adaboost <- function(test_data, ml_1, ml_2, ml_3) {
  ...
  w <- rep(1 / nrow(test_data), nrow(test_data))
  
  # Select samples
  obs <- sample_n(test_data, size = nrow(test_data), weight = w, replace = T)
  ...
  obs2 <- sample(test_data, prob = nw, replacement = T)
  
  p2 <- predict(ml_2, obs2)
  
  e2 <- sum(w2[p2 == test_data$classes])
  
  alpha <- (1 / 2) * ln((1 - e2) / e2)
