...
for (i in 1:max_k) {
  ...
  bc_knn <- knn(train = dplyr::select(bc_train.oversampled, -cancer_type),
                ...
                cl = factor(bc_train.oversampled$cancer_type, levels = bc_order), 
                k = i)
  
  # Create CrossTable
  ct <- CrossTable(x = factor(bc_test$cancer_type, levels = bc_order), 
                   y = factor(bc_knn, levels = bc_order), prop.chisq = F)$t
  
  # Find Accuracy
  accuracy[i] <- findAccuracy(ct)
  
  # Find AUC
  auc_vals[i] <- auc(as.numeric(bc_test$cancer_type),
...
ann.k <- c(unlist(ann.k)[[1]], unlist(ann.k)[[3]], unlist(ann.k)[[5]],
           unlist(ann.k)[[7]], unlist(ann.k)[[9]], unlist(ann.k)[[11]],
           unlist(ann.k)[[13]], unlist(ann.k)[[15]], unlist(ann.k)[[17]],
           unlist(ann.k)[[19]])
